...
function renderMemoPdf(data) {
  ...
  const typeBody = {
    ...
    dep: `เนื่องด้วยพนักงานโครงการ ${esc(data.project||'-')} - บริษัท ออร์บิท ดิจิทัล จำกัด${data.depLocation ? ` สาขา${esc(data.depLocation)}` : ''} มีแผนการ Deployment ระบบ${data.depStart ? ` ในช่วงวันที่ ${esc(data.depStart)}${data.depEnd && data.depEnd !== data.depStart ? ` – ${esc(data.depEnd)}` : ''}` : ''} จึงขออนุมัติงบประมาณค่าใช้จ่ายในการ Deployment ตามรายละเอียดดังต่อไปนี้`,
  };
  ...
  const closingMap = {
    ...
    dep: data.total ? `จึงขอความอนุเคราะห์ท่านโปรดพิจารณาอนุมัติค่าใช้จ่ายสำหรับรายการ Deployment ข้างต้น ในวงเงิน ${amtStr} ถ้าอ้างอิงอำนาจอนุมัติจากคู่มืออำนาจอนุมัติ พ.ศ. 2566 ข้อ 3.2 การชำระเงิน (ที่มีการตั้งงบประมาณไว้) หมวดการชำระค่าสวัสดิการพนักงาน ซึ่งให้อำนาจแก่ผู้บริหารในการอนุมัติการชำระเงินในวงเงินไม่เกิน 500,000 บาท` : '',
  };
